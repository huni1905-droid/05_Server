...
<body>
	...
	<pre>
		...
		-> 서버에 접속한 클라이언트를 나타내거나, 
			...
			(session 객체는 "서버에서 관리"함)
			
		-> session 은 브라우저 마다 하나씩 생성된다!!!
		-> 사이트 접속 ~브라우저 종료 | 세션 시간 만료 까지 유효하다(유지된다)	
		...
		-> 서버 실행 ~ 서버 종료 까지 유효하다(유지된다)		
		-> application 에 세팅된 값을 어디서든 사용 가능.
	</pre>
	...
	// pageValue=page scope 객체에 세팅한 값 (K=V 형태)
	%>
	...
	<%
		pageContext.setAttribute("메뉴", "짬짜면(page)");
	%>
