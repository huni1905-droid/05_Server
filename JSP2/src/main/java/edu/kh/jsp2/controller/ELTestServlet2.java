...
public class ELTestServlet2 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		// 1. page scope -> JSP에서만 확인가능 (딱 1페이지만 유효)
		
		
		// 2. request scope
		//	- 요청 받은 Servlet과 요청이 위임된 JSP까지 유지되는 객체
		
		// 2-1) 객체의 값(속성) 추가하는 방법
		//		범위객체.setAttribute("key", value);
		
		// 2-2) 객체에서 값(속성) 얻어오는 방법
		...
		// 3-1) session scope 객체 얻어오기
		HttpSession session = req.getSession();
		
		// 3-2) session scope 값 세팅해보기
		session.setAttribute("sessionValue", "session scope 객체에 세팅한 값");
		// sessionValue=session scope 객체에 세팅한 값( K=V 형태 )
		...
		// 4-1) application scope 얻어오기
		ServletContext application = req.getServletContext();
		
		// 4-2) application scope 값 세팅
